...
- The original `start_date` remains the feed's identity boundary. `morning_start_date` controls the active question launch; earlier CSV rows have phase `cancelled` and zero quotas. Cancelled events are transparent and shown separately on the website. Video cancellations use kind `cancelled`, no lessons, and the original date-based UID.
