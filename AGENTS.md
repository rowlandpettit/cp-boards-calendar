...
- Preserve each study occurrence's date-based UID. For cancellation, retain the event with `status = "CANCELLED"` unless the user explicitly asks to remove it entirely. In that case add its anchor to `settings.removed_occurrences` to omit it from all published outputs while keeping source history. Never remove active work silently.
